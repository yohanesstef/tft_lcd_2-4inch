...
//	GPIO_SetPinMode(GPIOA, GPIO_PIN_3, GPIO_MODE_OUTPUT_PP);
//	GPIO_SetPinMode(GPIOA, GPIO_PIN_5, GPIO_MODE_OUTPUT_PP);
//	GPIO_SetPinMode(GPIOB, GPIO_PIN_6, GPIO_MODE_INPUT);
//	ADC_GPIOA_init(GPIO_PIN_2);
//
//	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_3, GPIO_PIN_RESET);
//	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);

	GPIO_SetPinMode(XM_Port, XM_Pin, GPIO_MODE_OUTPUT_PP);
	GPIO_SetPinMode(XP_Port, XP_Pin, GPIO_MODE_OUTPUT_PP);
	GPIO_SetPinMode(YM_Port, YM_Pin, GPIO_MODE_INPUT);
	ADC_GPIOA_init(YP_Pin);
...
//	GPIO_SetPinMode(GPIOB, GPIO_PIN_6, GPIO_MODE_OUTPUT_PP);
//	GPIO_SetPinMode(GPIOA, GPIO_PIN_2, GPIO_MODE_OUTPUT_PP);
//	GPIO_SetPinMode(GPIOA, GPIO_PIN_5, GPIO_MODE_INPUT);
//	ADC_GPIOA_init(GPIO_PIN_3);
//
...
//	HAL_NVIC_EnableIRQ(EXTI2_IRQn);
	...
	HAL_NVIC_DisableIRQ(YP_EXTI);
	GPIO_SetPinMode(YM_Port, YM_Pin, GPIO_MODE_OUTPUT_PP);
	GPIO_SetPinMode(YP_Port, YP_Pin, GPIO_MODE_OUTPUT_PP);
	GPIO_SetPinMode(XP_Port, XP_Pin, GPIO_MODE_INPUT);
	ADC_GPIOA_init(XM_Pin);
	...
	__HAL_GPIO_EXTI_CLEAR_IT(YP_Pin);
	HAL_NVIC_ClearPendingIRQ(YP_EXTI);
	HAL_NVIC_EnableIRQ(YP_EXTI);
...
static void GPIO_InterruptMode() {
	...
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(XM_Port, &GPIO_InitStruct);
	HAL_GPIO_WritePin(XM_Port, XM_Pin, GPIO_PIN_RESET);

	/* X+ PA5 *///ubah dikit ganti ke pin A5
	GPIO_InitStruct.Pin = XP_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(XP_Port, &GPIO_InitStruct);

	/* Y- PB6 *///ubah dikit ganti ke pin B6
	GPIO_InitStruct.Pin = YM_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(YM_Port, &GPIO_InitStruct);

	/* Y+ PA4 */
	GPIO_InitStruct.Pin = YP_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING_FALLING;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	HAL_GPIO_Init(YP_Port, &GPIO_InitStruct);
	HAL_NVIC_SetPriority(YP_EXTI, 0, 0);
	HAL_NVIC_EnableIRQ(YP_EXTI);
}
